...
function BusinessDashboard() {
    ...
    const [settings, setSettings] = useState(() => {
        ...
        return saved ? JSON.parse(saved) : businessSettingsData;
    });
    ...
    const loadDashboardData = async ({ showLoading = true } = {}) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    data.error || "Error cargando panel del negocio"
                );
            }
            ...
            setError(error.message || "Error cargando panel del negocio");
        } finally {
            if (showLoading) {
                setLoading(false);
            }
    ...
    async function updateReservationStatus(reservationId, status) {
        try {
            ...
            if (!response.ok) {
                throw new Error(
                    data.error || "No se pudo actualizar la reserva"
                );
            }
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        <div>
                            ...
                            <h1 className="mt-5 text-4xl md:text-5xl font-semibold tracking-tight">
                                {business?.name || "Administración de tienda"}
                            </h1>

                            <p className="mt-4 text-zinc-400 max-w-2xl leading-relaxed">
                                Gestiona reservas, horarios, clientes,
                                profesionales, servicios y seguridad desde un
                                solo lugar.
                            </p>
                        </div>
